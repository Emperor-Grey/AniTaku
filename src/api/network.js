...
export async function getRecentEpisodes() {
    try {
        const data = await fetchData('meta/anilist/recent-episodes?page=1&perPage=21&provider=gogoanime');
    ...
    try {
        ...
        return {
            anime: {
                ...
                duration: data.duration, // Add additional properties here
                season: data.season,
...
export async function getAnimeDataByGenre(genre, page = 1, perPage = 15) {
    try {
        const data = await fetchData(`meta/anilist/advanced-search?genres=["${genre}"]&page=${page}&perPage=${perPage}`);
        return data.results.map(anime => ({
            id: anime.id,
            title: anime.title,
            image: anime.image,
...
export async function getSchedule() {
    try {
        ...
        const url = 'https://api.anify.tv/schedule?type=anime&fields=[id,title,coverImage,rating]';
...
export async function searchAnime(query, page) {
    try {
        ...
        const searchInfo = {
            total: searchData.total, lastPage: searchData.lastPage, results,
        };
