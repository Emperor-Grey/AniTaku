...
const EpisodeRow = ({data, mainData, more}) => {
    ...
    const onPressItem = async item => {
        ...
        const isSelected = selectedEpisodes.some(selected => selected.id === item.id);

        if (isSelected) {
            setSelectedEpisodes(prevSelected => prevSelected.filter(selected => selected.id !== item.id));
        } else {
            setSelectedEpisodes(prevSelected => [...prevSelected, item]);
        }

        try {
            await AsyncStorage.setItem('selectedEpisodes', JSON.stringify(selectedEpisodes));
        } catch (error) {
    ...
    useEffect(() => {
        const retrieveSelectedEpisodes = async () => {
            try {
                const storedSelectedEpisodes = await AsyncStorage.getItem('selectedEpisodes');
    ...
    const renderItem = ({item}) => {
        ...
        return (<TouchableOpacity
                ...
                <ImageBackground
                    ...
                    {isSelected && (<View style={styles.overlayContainer}>
                            <LinearGradient
...
const styles = StyleSheet.create({
    episodeItem: {
        ...
        width: responsiveWidth(43), resizeMode: 'cover', height: responsiveHeight(15),
    }, overlayContainer: {
        ...StyleSheet.absoluteFillObject, justifyContent: 'center', alignItems: 'center',
    }, overlayGradient: {
        width: '100%', height: '100%', justifyContent: 'center', alignItems: 'center',
    }, episodeTitle: {
        color: 'white', fontSize: 16, fontWeight: 'bold', paddingRight: 7,
    },
});
