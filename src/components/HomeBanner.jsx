...
export default function HomeBanner({data}) {
    ...
    return (<View style={style.imageContainer}>
            <ImageBackground
                ...
                <LinearGradient
                    className="flex-1"
                    colors={['rgba(0,0,0,0.17)', 'rgba(0,0,0,0.09)', 'rgba(10, 10, 10,0.95)', 'rgba(10, 10, 10,1.2)']}
                    ...
                    <View className="justify-between flex-1 mb-12">
                        ...
                        <View className="items-center justify-center">
                            ...
                            <View className="flex-row items-center justify-center mb-3">
                                {data.genres.map((genre, index) => (<React.Fragment key={index}>
                                        <Text className="text-gray-300">{genre}</Text>
                                        {index < data.genres.length - 1 && (<Text className="text-gray-300">, </Text>)}
                                    </React.Fragment>))}
                            </View>
                            <View className="flex-row justify-evenly px-16 gap-5">
                                ...
                                <TouchableOpacity
                                    className="flex-1 bg-transparent border flex-row border-solid border-gray-500 p-3 rounded-3xl space-x-2 items-center justify-center">
...
const style = StyleSheet.create({
    imageContainer: {
        height: responsiveHeight(63.5), width: responsiveWidth(100),
    },
});
