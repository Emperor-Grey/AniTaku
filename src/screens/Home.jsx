...
import {getAnimeDataByGenre, getPopularData, getRecentEpisodes, getTrendingData} from '../api/network';
...
const Home = () => {
        ...
        const fetchData = async () => {
            try {
                ...
                for (let i = shuffledPopularData.length - 1; i > 0; i--) {
                    ...
                    [shuffledPopularData[i], shuffledPopularData[j]] = [shuffledPopularData[j], shuffledPopularData[i]];
                }
    ...
    return (<View className="bg-neutral-950 flex-1">
            <StatusBar
                translucent
            ...
            <ScrollView showsVerticalScrollIndicator={false}>
                ...
                {isLoading ? (<View className="flex-1 justify-center items-center mt-[85%]">
                        <LottieView
                        ...
                        {genres.map(genre => (<RowItem key={genre} name={genre} data={genreData[genre] || []}/>))}
                    </>)}
            </ScrollView>
            <Fab/>
        </View>);
